...
locals {
  ...
  kql_database_redis_resources_details = templatefile(
    ...
    kql_database_postgresql_resources_details = templatefile(
    "${path.module}/template_kql/database/database_postgresql_resources_details.kql",
    {
      "extend_resource" = local.kql_extend_resource
    }
  )
  ...
  workbook_databases_json = templatefile("${path.module}/templates/databases.tpl.json", {
    ...
    "kql_database_postgresql_resources_details"    = jsonencode(local.kql_database_postgresql_resources_details)
  })
}
